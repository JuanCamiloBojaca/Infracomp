...
public class Delegado implements Runnable {
	...
	Delegado(Socket csP, int idP, PrintStream file) {
		this.file = file;
		sc = csP;
	...
	public void run() {
		...
		try {
			...
			if (!algoritmos[1].equals(Seg.DES) && !algoritmos[1].equals(Seg.AES) && !algoritmos[1].equals(Seg.BLOWFISH)
					&& !algoritmos[1].equals(Seg.RC4)) {
				ac.println(ERROR);
				sc.close();
